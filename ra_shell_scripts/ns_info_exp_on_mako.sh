...
/home/ychang/bin/mako --csv -s "select ip, demand, pingpoint, country, city, asnum, rank() over (partition by country order by demand desc) rankings from ns_info where country='JP' and demand<0.3 and rownum<=1 " | tail -1 > $temp_file
